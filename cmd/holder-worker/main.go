...
func init() {
	...
	flag.Int64Var(&_flag.cashier.capacity, "cashier.capacity", 10, "custom capacity for worker cashier")

	// worker.datadog.Config
	flag.DurationVar(&_flag.datadog.interval, "datadog.interval", time.Minute, "custom datadog trigger interval")
}
